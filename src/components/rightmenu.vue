<template>
  <ul class="vue-contextmenu-listWrapper" :class="'vue-contextmenuName-' + contextMenuData.menuName">
    <li v-for="item in contextMenuData.menulists"
      ...
      :key="item.btnName">
        <el-button @click.stop="fnHandler(item)" size="mini" :icon="item.icoName">{{item.btnName}}</el-button>
    </li>
  </ul>
</template>
<script>
...
<style>
  ...
  .vue-contextmenu-listWrapper .context-menu-list button:hover {
    ...
    color: #ffffff;
    border-radius: 4px;
    background: -webkit-linear-gradient(bottom, #cecaca , #b7babd); /* Safari 5.1 - 6.0 */
    background: -o-linear-gradient(bottom, #cecaca, #b7babd); /* Opera 11.1 - 12.0 */
    background: -moz-linear-gradient(bottom, #cecaca, #b7babd); /* Firefox 3.6 - 15 */
